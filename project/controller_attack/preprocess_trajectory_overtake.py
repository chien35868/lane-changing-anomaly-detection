...
df = pandas.read_csv("./trajectory/overtake_3/acceleration_vehicle_attack/with_attack/raw_trajectory.csv", on_bad_lines='skip')
df.columns = header_list
df = df.sort_values(['vid', 'time'], ascending=[True, True])
df.to_csv("./trajectory/overtake_3/acceleration_vehicle_attack/with_attack/raw_trajectory.csv", mode='w+', index=False, header=True)
...
for vid in df['vid'].unique():
    ...
    lanechange_df = target_df[(target_df["left_lanechange_state"].apply(lambda x: "'left'" in x)) & (target_df["lane_id"]=="E0_1")] #for overtake_3
...
for vid in df['vid'].unique():
    ...
    lanechange_df = target_df[(target_df["right_lanechange_state"].apply(lambda x: "'right'" in x)) & (target_df["lane_id"]=="-E0_1")] #for overtake_3
...
for dataframe in dataframe_set:
    ...
    if dataframe.equals(left_leader_anomaly_blocked_df) or dataframe.equals(left_follower_anomaly_blocked_df) or dataframe.equals(left_leader_normal_blocked_df) or dataframe.equals(left_follower_normal_blocked_df):
        ...
        if store_key == 0:
            dataframe.to_csv('./trajectory/overtake_3/acceleration_vehicle_attack/with_attack/anomaly_blocked_attack.csv', mode='a+', index=False, header=False) 
        else:
            dataframe.to_csv('./trajectory/overtake_3/acceleration_vehicle_attack/with_attack/normal_blocked_attack.csv', mode='a+', index=False, header=False) 
