...
#include "/home/r10922a06/Desktop/sumo2/sumo/src/libsumo/Simulation.h"
#include "/home/r10922a06/Desktop/sumo2/sumo/src/libsumo/TraCIConstants.h"
#include "/home/r10922a06/Desktop/sumo2/sumo/src/utils/common/SUMOTime.h"


#include "MSGlobal.h"
// unordered_map<string, unordered_map<string, vector<double>>> record;
// int load_key = 0;
...
public:
    ...
    double getPositionOnLane() const {
        // cout << vehicle_id << " get vid, " << getID() << " position" << endl;
        if (vehicle_id == ""){
            return myState.myPos;
        }else{
            if (getID() == vehicle_id){
                // cout << "self get self, " << vehicle_id << " vehicle call: "   << getID() << "'s position:" << myState.myPos << endl; 
                return myState.myPos;
            }else{
                if (getVehicleType().getID() == "anomaly"){
                    // mt19937 gen(myState.mySpeed + myState.myPos); // create the seed that is being unique to every vehicle
                    // normal_distribution<> distr(0, 15);
                    // double speed_with_attack = myState.mySpeed + distr(gen);
                    // return speed_with_attack;

                    // cout << "self get other attack, " << vehicle_id << " vehicle call: "  << getID() << "'s position:" << myState.myPos + attack_pos() << endl; 
                    // cout << SIMTIME << ", attack_pos = " << attack_pos() << endl;

                    // return myState.myPos + attack_pos();
                    // return myState.myPos;
                    return attack_pos_ghost();          
                }else{
                    // cout << "self get other normal, " << vehicle_id << " vehicle call: "  << getID() << "'s position: " << myState.myPos << endl; 
                    return myState.myPos;
                }   
            }            
        }
    ...
    double getPositionOnLane_real() const{
        return myState.myPos;
    }
    ...
    inline double getBackPositionOnLane(const MSLane* lane) const {
        // cout << "id:" << getID() << " backposition:" << getBackPositionOnLane(lane, false) <<endl;
        if (vehicle_id == ""){
            ...
            if (getID() == vehicle_id){
                ...
                if (getVehicleType().getID() == "anomaly"){
                    ...
                    // return getBackPositionOnLane(lane, false) + attack_pos();
                    return attack_pos_ghost()-5;
                    // return getBackPositionOnLane(lane, false);
                }else{
                    // cout << "self get other normal, " << vehicle_id << " vehicle call: "  << getID() << "'s position: " << myState.myPos << endl; 
                    return getBackPositionOnLane(lane, false);
    ...
    double getBackPositionOnLane_real(const MSLane* lane) const{
        return getBackPositionOnLane(lane, false);
    }
    ...
    void load_data() const{
        if (load_key == 0){
            std::ifstream inFile;
            inFile.open("/home/r10922a06/Desktop/sumo2/sumo/project/controller_attack/attack_file/ghost_attack.txt");

            string vehicle, time;
            string pos_, speed_, acc_;
            double pos, speed, acc;
            while(inFile >> vehicle >> time >> pos_ >> speed_ >> acc_){

                pos = std::stod(pos_);
                speed = std::stod(speed_);
                acc = std::stod(acc_);
                record[vehicle][time] = {pos, speed, acc};
            }
            load_key += 1;
        }
    }

    double attack_pos_ghost() const {

        double offset = 0.1;
        double origin_simtime = SIMTIME;

        
        string simtime_ = to_string(origin_simtime-offset);
        string origin_simtime_ = to_string(origin_simtime);
        simtime_.erase(simtime_.end()-5, simtime_.end());
        origin_simtime_.erase(origin_simtime_.end()-5, origin_simtime_.end());

        load_data();
        string anomaly_id = getID();
        double pos;
        if(record[anomaly_id].count(simtime_)>0){
            good += 1;
            // cout << good << endl;
            pos = record[anomaly_id][simtime_][0];
            return pos;
        }else{
            bad += 1;
            // cout << bad << endl;
            return myState.myPos;
            // cout << "wrong" << endl;
            // cout << anomaly_id << "\t" << origin_simtime_ << endl;
            // pos = record[anomaly_id][origin_simtime_][0];
        }
        
        // cout << pos << "\t" << speed << "\t" << acc << endl;

        
    }

    double attack_vel_ghost() const {
        double offset = 0.1;
        double origin_simtime = SIMTIME;
        ...
        if(record[anomaly_id].count(simtime_)>0){
            good += 1;
            // cout << good << endl;
            vel = record[anomaly_id][simtime_][1];
            return vel;
        }else{
            bad += 1;
            // cout << bad << endl;
    ...
    double attack_accel_ghost() const {
        double offset = 0.1;
        double origin_simtime = SIMTIME;
        ...
        if(record[anomaly_id].count(simtime_)>0){
            good += 1;
            // cout << good << endl;
            acc = record[anomaly_id][simtime_][1];
            return acc;
        }else{
            bad += 1;
            // cout << bad << endl;
        ...
        cout << "Current time step" << origin_simtime_ << " get vehicle " << getID() << "at time step " << simtime_ << " is " << acc << endl;
        return acc;
    }
    double attack_accel() const {
        ifstream inFile;
        inFile.open("/home/r10922a06/Desktop/sumo2/sumo/project/controller_attack/attack_file/attack_acc.txt");
        float acc[200] = {0};
        for(int i=0; i<200; i++){
            inFile >> acc[i];
        }
        int time = SIMTIME*10;
        // cout << "attack_acc= " << acc[time%200] <<endl;
        // cout <<  << acc[time%200] <<endl;
        return acc[time%200];
        // return 0;

    }

    double attack_vel() const { //SIMTIME is increased by 0.2
        ifstream inFile;
        inFile.open("/home/r10922a06/Desktop/sumo2/sumo/project/controller_attack/attack_file/attack_vel.txt");
        float vel[200] = {0};
        for(int i=0; i<200; i++){
            inFile >> vel[i];
        }
        int time = SIMTIME*10;
        // cout << "attack_vel= " << vel[time%200] << endl;
        return vel[time%200];
        // return 0;
    }

    double attack_pos() const {
        ifstream inFile;
        inFile.open("/home/r10922a06/Desktop/sumo2/sumo/project/controller_attack/attack_file/attack_pos.txt");
        float pos[200] = {0};
        for(int i=0; i<200; i++){
            inFile >> pos[i];
        }
        int time = SIMTIME*10;
        // cout << SIMTIME << "\t" << time << endl;
        // cout << "attack_pos= " << pos[time%200] << endl;
        return pos[time%200];
        // return 0;
    }
    // vehicle_id is the original vehicle caller at the beginningsimulation, 
    // and the getNumericalID() is the target vehicle of the function which called afterward 
    double getSpeed() const {
        if (vehicle_id == ""){
            ...
            if (getID() == vehicle_id){
                ...
                if (getVehicleType().getID() == "anomaly"){
                    // cout << "time:" << SIMTIME << "self get other attack, " << vehicle_id << " vehicle call: "  << getID() << "'s position:" << myState.myPos + attack_pos() << endl; 
                    // return myState.mySpeed + attack_vel();
                    // return myState.mySpeed;   
                    return attack_vel_ghost();        
                }else{
                    // cout << "self get other normal, " << vehicle_id << " vehicle call: "  << getID() << "'s position: " << myState.myPos << endl; 
                    return myState.mySpeed;
                }   
    ...
    //     //     cout<<"normal ";
    //     // }
    //     // cout<<abs(get_anomaly_Speed() - get_noise_Speed()/get_noise_Speed())<<endl;
    //     if(abs(get_anomaly_Speed() - get_noise_Speed()/get_noise_Speed()) < 0.04){
    //         return get_anomaly_Speed();
    //     }else{
    //         return get_noise_Speed();
    //     }
    // }



    // normal and anomaly vehicles may get the wrong information from anomaly vehicle.
    double getSpeed_may_anomaly() const {
        if (getVehicleType().getID() == "anomaly"){
            // mt19937 gen(myState.mySpeed + myState.myPos); // create the seed that is being unique to every vehicle
            // normal_distribution<> distr(0, 15);
            // double speed_with_attack = myState.mySpeed + distr(gen);
            // return speed_with_attack;
            return getSpeed_safe() + attack_vel();          
        }else{
            return getSpeed_safe();
        }
    }

    // anomaly and normal vehicle would not get the wrong information from itself
    double getSpeed_safe() const {
        mt19937 gen(myState.mySpeed + myState.myPos); // create the seed that is being unique to every vehicle
        normal_distribution<> distr(0, 0.2);
        double speed_with_noise = myState.mySpeed + distr(gen);
        return speed_with_noise;
    }
        ...
        if (vehicle_id == ""){
            ...
            if (getID() == vehicle_id){
                ...
                if (getVehicleType().getID() == "anomaly"){
                    ...
                    // return myAcceleration + attack_accel(); 
                    return attack_accel_ghost();
                    // return myAcceleration;          
                }else{
                    // cout << "self get other normal, " << vehicle_id << " vehicle call: "  << getID() << "'s position: " << myState.myPos << endl; 
                    return myAcceleration;
