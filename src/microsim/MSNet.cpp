...
MSNet::simulationStep() {
    // cout <<"one simulation step start: " << myStep <<endl;
#ifdef DEBUG_SIMSTEP
    ...
    if (MSGlobals::gUseMesoSim) {
        ...
        // cout << "patchActiveLanes()" <<endl;
        myEdges->patchActiveLanes(); //done //no need to implement
        ...
        myEdges->planMovements(myStep); //done
    ...
    // normal_or_anomaly = 1; //1 means anomaly vehicle, 0 means normal vehicle
    vehicle_id = "";
...
#endif
    // cout<<"2.1"<<endl;
    myInserter->emitVehicles(myStep); // may be called sometimes
    // cout<<"2.2"<<endl;
    if (MSGlobals::gCheck4Accidents && !MSGlobals::gUseMesoSim) {
    ...
    myEndOfTimestepEvents->execute(myStep); //every one second in simulation would call this function
    // cout<<"3.1"<<endl;
    if (myLogExecutionTime) {
        myTraCIStepDuration -= SysUtils::getCurrentMillis();
    }
    ...
    myStep += DELTA_T;
    // cout <<"one simulation step end" <<endl;
}
