...
// ===========================================================================
MSLCM_LC2022::MSLCM_LC2022(MSVehicle& v) :
    MSAbstractLaneChangeModel(v, LaneChangeModel::LC2022),
...
MSLCM_LC2022::~MSLCM_LC2022() {
    changed();
}


void
MSLCM_LC2022::initDerivedParameters() {
...
bool
MSLCM_LC2022::debugVehicle() const {
    return DEBUG_COND;
}


int
MSLCM_LC2022::wantsChange(
    int laneOffset,
...
double
MSLCM_LC2022::patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
...
double
MSLCM_LC2022::_patchSpeed(double min, const double wanted, double max, const MSCFModel& cfModel) {
...
void*
MSLCM_LC2022::inform(void* info, MSVehicle* sender) {
...
double
MSLCM_LC2022::overtakeDistance(const MSVehicle* follower, const MSVehicle* leader, const double gap, double followerSpeed, double leaderSpeed) {
...
double
MSLCM_LC2022::informLeader(MSAbstractLaneChangeModel::MSLCMessager& msgPass,
                           int blocked,
                           int dir,
                           const std::pair<MSVehicle*, double>& neighLead,
...
void
MSLCM_LC2022::informFollower(MSAbstractLaneChangeModel::MSLCMessager& msgPass,
                             int blocked,
                             int dir,
                             const std::pair<MSVehicle*, double>& neighFollow,
                             double remainingSeconds,
...
double
MSLCM_LC2022::getExtraReservation(int bestLaneOffset) const {
    if (bestLaneOffset < -1) {
        return 20;
...
double
MSLCM_LC2022::anticipateFollowSpeed(const std::pair<MSVehicle*, double>& leaderDist, double dist, double vMax, bool acceleratingLeader) {
...
int
MSLCM_LC2022::slowDownForBlocked(MSVehicle** blocked, int state) {
...
void
MSLCM_LC2022::adaptSpeedToPedestrians(const MSLane* lane, double& v) {
...
void MSLCM_LC2022::addLCSpeedAdvice(const double vSafe) {
...
double
MSLCM_LC2022::computeSpeedLat(double latDist, double& maneuverDist, bool urgent) const {
...
double
MSLCM_LC2022::getAssumedDecelForLaneChangeDuration() const {
    return MAX2(LC_ASSUMED_DECEL, -myVehicle.getAcceleration());
}

double
MSLCM_LC2022::getSafetyFactor() const {
    return 1 / myAssertive;
}

double
MSLCM_LC2022::getOppositeSafetyFactor() const {
    return myOppositeParam <= 0 ? std::numeric_limits<double>::max() : 1 / myOppositeParam;
}

bool
MSLCM_LC2022::saveBlockerLength(double length, double foeLeftSpace) {
...
std::string
MSLCM_LC2022::getParameter(const std::string& key) const {
...
void
MSLCM_LC2022::setParameter(const std::string& key, const std::string& value) {
    double doubleValue;
    try {
