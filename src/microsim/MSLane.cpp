...
MSLane::isInsertionSuccess(MSVehicle* aVehicle,
    ...
    if (aVehicle->hasStops()) {
        ...
        if (nextStop.lane == this) {
            ...
            if (checkFailure(aVehicle, speed, dist, MAX2(0.0, cfModel.stopSpeed(aVehicle, speed, distToStop)),
    ...
    for (int i = 0; i < followers.numSublanes(); ++i) {
        ...
        if (follower != nullptr) {
            ...
            if (followers[i].second < backGapNeeded
                    && ((aVehicle->getParameter().insertionChecks & (int)InsertionCheck::FOLLOWER_GAP) != 0
                        || (followers[i].second < 0 && (aVehicle->getParameter().insertionChecks & (int)InsertionCheck::COLLISION) != 0))) {
    ...
    if (shadowLane != nullptr) {
        ...
        for (int i = 0; i < shadowFollowers.numSublanes(); ++i) {
            ...
            if (follower != nullptr) {
                ...
                if (shadowFollowers[i].second < backGapNeeded
                        && ((aVehicle->getParameter().insertionChecks & (int)InsertionCheck::FOLLOWER_GAP) != 0
                            || (shadowFollowers[i].second < 0 && (aVehicle->getParameter().insertionChecks & (int)InsertionCheck::COLLISION) != 0))) {
                    // too close to the follower on this lane
#ifdef DEBUG_INSERTION
                    if (DEBUG_COND2(aVehicle) || DEBUG_COND) {
                        std::cout << SIMTIME
        ...
        for (int i = 0; i < ahead.numSublanes(); ++i) {
            ...
            if (veh != nullptr) {
                ...
                if (gap <  gapNeeded
                        && ((aVehicle->getParameter().insertionChecks & (int)InsertionCheck::LEADER_GAP) != 0
                            || (gap < 0 && (aVehicle->getParameter().insertionChecks & (int)InsertionCheck::COLLISION) != 0))) {
...
MSLane::planMovements(SUMOTime t) {
    ...
    for (; veh != myVehicles.rend(); ++veh) {
        // cout << "planMovements" << endl;
        // cout << (*veh)->getNumericalID() <<endl;
        vehicle_id = (*veh)->getID();
        // if ((*veh)->getVehicleType().getID() == "anomaly"){
        //     cout << "It is anomaly now!" << endl;
        //     normal_or_anomaly = 1;
        // }else{
        //     cout<<"It is normal now!"<<endl;
        //     normal_or_anomaly = 0;
        // }
#ifdef DEBUG_PLAN_MOVE
        if (DEBUG_COND2((*veh))) {
...
MSLane::setJunctionApproaches(const SUMOTime t) const {
    for (MSVehicle* const veh : myVehicles) {
        // cout << "setJunctionApproaches" << endl;
        vehicle_id = veh->getID();
        // if (veh->getVehicleType().getID() == "anomaly"){
        //     cout<<"It is anomaly now!"<<endl;
        //     normal_or_anomaly = 1;
...
MSLane::detectCollisions(SUMOTime timestep, const std::string& stage) {
    ...
    if (!MSGlobals::gSublane) { //the part I need to modify
        // no sublanes
        // for (VehCont::reverse_iterator pred = myVehicles.rbegin(); pred != myVehicles.rend(); ++pred) {
        //     cout << (*pred)->getID() << " "; 
        // }cout << endl;
        ...
        for (VehCont::reverse_iterator pred = myVehicles.rbegin(); pred != lastVeh; ++pred) {
            ...
            // cout << "veh " + (*veh)->getID() + " pred " + (*pred)->getID() << endl; 
...
// Function getBackPositionOnLane() and getPositionOnLane() is substitute by 
// getBackPositionOnLane_real() and getPositionOnLane_real()
// which the position it get would not be affected by the function caller.
bool
MSLane::detectCollisionBetween(SUMOTime timestep, const std::string& stage, MSVehicle* collider, MSVehicle* victim,
    ...
    // vehicle_id = "";
    // cout << "vehicle_id = " << vehicle_id << endl;
    // cout << SIMTIME << " " << "collider: " << collider->getID() << " " << collider->getBackPositionOnLane(this) << ", victim: "  << victim->getID() << " " << victim->getPositionOnLane() << endl;
    ...
    double victimBack = victimOpposite && !bothOpposite ? victim->getPositionOnLane(this) : victim->getBackPositionOnLane_real(this);
...
#endif
        // cout << vehicle_id << endl;
        // cout << timestep << " " << stage << " " << collider->getID() << " " << victim->getID() << " " << gap << " " << latGap << endl;
...
MSLane::handleCollisionBetween(SUMOTime timestep, const std::string& stage, const MSVehicle* collider, const MSVehicle* victim,
    ...
    if (newCollision) {
        ...
        if(stage == "laneChange"){
            ofstream myfile;
            myfile.open("/home/r10922a06/Desktop/sumo2/sumo/project/controller_attack/trajectory/collision_record.txt", fstream::app);
            myfile << prefix;
            myfile << endl;
        }
...
MSLane::checkForPedestrians(const MSVehicle* aVehicle, double& speed, double& dist,  double pos, bool patchSpeed) const {
    if (getEdge().getPersons().size() > 0 && hasPedestrians()) {
        ...
        if (leader.first != 0) {
            ...
            if ((gap < 0 && (aVehicle->getParameter().insertionChecks & ((int)InsertionCheck::COLLISION | (int)InsertionCheck::PEDESTRIAN)) != 0)
