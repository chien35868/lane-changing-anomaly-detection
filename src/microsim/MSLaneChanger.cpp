...
MSLaneChanger::laneChange(SUMOTime t) {
    ...
    try {
        while (vehInChanger()) {
            ...
            // cout << vehicle_id << " change(): " << haveChanged << endl;
            vehicle_id = "";
...
MSLaneChanger::updateChanger(bool vehHasChanged) {
    ...
    if (!vehHasChanged) {
        ...
        // std::cout << "position " << veh(myCandi)->myState.myPos << "\n";
...
MSLaneChanger::findCandidate() {
    ...
    // for (ChangerIt ce = myChanger.begin(); ce != myChanger.end(); ++ce){
    //     if(veh(ce)!=nullptr){
    //         cout <<"findcandidate " << veh(ce)->getID() << endl;
    //     }        
    // }
...
//        std::cout<< "\nCHANGE" << std::endl;
    ...
    MSVehicle* vehicle = veh(myCandi);// seems that the vehicles include all the vehicles on the lane
    // cout << SIMTIME << " " << vehicle->getID() << endl;
    // cout << "change" << endl;
    
    // the severe error done my myself
    vehicle_id = vehicle->getID();
    // cout << vehicle_id << endl;
    
    // if (vehicle->getVehicleType().getID() == "anomaly"){
    ...
    if (vehicle->getLaneChangeModel().isChangingLanes() && !vehicle->getLaneChangeModel().alreadyChanged()) { // not useful
        return continueChange(vehicle, myCandi);
    }
    vehicle->getLaneChangeModel().setSpeedLat(0);
    if (!myAllowsChanging || vehicle->getLaneChangeModel().alreadyChanged() || vehicle->isStoppedOnLane()) { // not useful
    ...
    if (!vehicle->isActive()) { // not useful
#ifdef DEBUG_ACTIONSTEPS
        if (DEBUG_COND) {
    ...
    if (myChanger.size() == 1 || vehicle->getLaneChangeModel().isOpposite() || (!mayChange(-1) && !mayChange(1))) {// not useful
    ...
    if ((stateRight & LCA_URGENT) != 0 && (stateLeft & LCA_URGENT) != 0) { // not useful
    ...
    if ((vehicle->getVehicleType().getVehicleClass() == SVC_EMERGENCY
            ...
            && changeOpposite(vehicle, leader, myCandi->lastStopped)) { // not useful
        return true;
    }

    // if the vehicle do not change the lane, the process would get here.
    registerUnchanged(vehicle);
    return false;
}
...
MSLaneChanger::checkChangeWithinEdge(
    ...
    //     cout << vehicle_id << " " <<  neighFollow.first->getID() << " " << neighFollow.second << endl;
    // }
...
MSLaneChanger::checkChange(
    ...
    // cout << laneOffset << blockedByLeader <<endl;
    int blockedByFollower = (laneOffset == -1 ? LCA_BLOCKED_BY_RIGHT_FOLLOWER : LCA_BLOCKED_BY_LEFT_FOLLOWER);
    // overlap
    // cout << neighFollow.second << endl;
    if (neighFollow.first != nullptr && neighFollow.second < 0) {
        // cout << "neighFollow.first: " << neighFollow.first->getID() << " neighFollow.second: " << neighFollow.second << endl; 
    ...
    if (neighLead.first != nullptr && neighLead.second < 0) {
        // cout << "neighLead.first: " << neighLead.first->getID() << " neighLead.second: " << neighLead.second << endl; 
    ...
    // if(blocked!=0){
    //     cout << "blocked:" << vehicle->getID() <<endl;
    // }
    ...
    // cout << SIMTIME << ", " << vehicle_id << ", " << state << endl;
    return state;
}
